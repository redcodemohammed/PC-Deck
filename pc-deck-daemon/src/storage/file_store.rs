...
impl FileStore {
 ...
 pub fn find_button(&self, deck_id:&str, button_id:&str)->Result<Option<crate::models::DeckButton>,AppError>{
     let db=self.read_decks()?;
     Ok(db.decks.into_iter().find(|d|d.id==deck_id).and_then(|d|d.buttons.into_iter().find(|b|b.id==button_id)))
 }
